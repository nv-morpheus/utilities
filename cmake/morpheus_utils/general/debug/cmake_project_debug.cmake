...
function(morpheus_utils_print_config)
    ...
    # There are two types of verbosity in CMake:
    #   1. Configure time verbosity: This controls the level at which `message()` commands are printed. This is
    #      controlled by the `CMAKE_MESSAGE_LOG_LEVEL`. To change the verbosity level during configuration, use the
    #      `--loglevel` option to the `cmake` command. For example, to print all messages, use
    #      `cmake --loglevel=VERBOSE`.
    #      Note: The `VERBOSE` option to the `message()` command is only used to control the verbosity of the message
    #      being printed. It does not control the verbosity of the `message()` command itself.
    #   2. Build time verbosity: This controls the verbosity of the build system. For example, when using Makefiles with
    #      verbose output, every command would be printed to the console. Not just failures and warnings. This is
    #      controlled by the `CMAKE_VERBOSE_MAKEFILE` variable or by setting the environment variable `VERBOSE=1`.
    #
    # It is NOT necessary to check the `VERBOSE` variable before executing this function!
    ...
    message(VERBOSE "CMAKE_FIND_ROOT_PATH_MODE_LIBRARY: " ${CMAKE_FIND_ROOT_PATH_MODE_LIBRARY})
    message(VERBOSE "CMAKE_FIND_ROOT_PATH_MODE_PACKAGE: " ${CMAKE_FIND_ROOT_PATH_MODE_PACKAGE})
    message(VERBOSE "CMAKE_FIND_ROOT_PATH_MODE_PROGRAM: " ${CMAKE_FIND_ROOT_PATH_MODE_PROGRAM})
    message(VERBOSE "CMAKE_FIND_ROOT_PATH: " ${CMAKE_FIND_ROOT_PATH})
    message(VERBOSE "CMAKE_FIND_USE_CMAKE_ENVIRONMENT_PATH: " ${CMAKE_FIND_USE_CMAKE_ENVIRONMENT_PATH})
    message(VERBOSE "CMAKE_FIND_USE_CMAKE_PATH: " ${CMAKE_FIND_USE_CMAKE_PATH})
    message(VERBOSE "CMAKE_FIND_USE_CMAKE_SYSTEM_PATH: " ${CMAKE_FIND_USE_CMAKE_SYSTEM_PATH})
    message(VERBOSE "CMAKE_FIND_USE_INSTALL_PREFIX: " ${CMAKE_FIND_USE_INSTALL_PREFIX})
    message(VERBOSE "CMAKE_FIND_USE_PACKAGE_REGISTRY: " ${CMAKE_FIND_USE_PACKAGE_REGISTRY})
    message(VERBOSE "CMAKE_FIND_USE_PACKAGE_ROOT_PATH: " ${CMAKE_FIND_USE_PACKAGE_ROOT_PATH})
    message(VERBOSE "CMAKE_FIND_USE_SYSTEM_ENVIRONMENT_PATH: " ${CMAKE_FIND_USE_SYSTEM_ENVIRONMENT_PATH})
    message(VERBOSE "CMAKE_FIND_USE_SYSTEM_PACKAGE_REGISTRY: " ${CMAKE_FIND_USE_SYSTEM_PACKAGE_REGISTRY})
    ...
    message(VERBOSE "CMAKE_SYSTEM_INCLUDE_PATH: " ${CMAKE_SYSTEM_INCLUDE_PATH})
    message(VERBOSE "CMAKE_SYSTEM_LIBRARY_PATH: " ${CMAKE_SYSTEM_LIBRARY_PATH})
    message(VERBOSE "ENV{CMAKE_FRAMEWORK_PATH}: " $ENV{CMAKE_FRAMEWORK_PATH})
    message(VERBOSE "ENV{CMAKE_INCLUDE_PATH}: " $ENV{CMAKE_INCLUDE_PATH})
    message(VERBOSE "ENV{CMAKE_LIBRARY_PATH}: " $ENV{CMAKE_LIBRARY_PATH})
